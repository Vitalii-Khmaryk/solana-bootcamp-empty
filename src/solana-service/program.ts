...
export class EscrowProgram {
  ...
  async makeOffer(
    ...
    const txInstruction = await this.program.methods.makeOffer(offerId,new BN(tokenAmountA),new BN(tokenAmountB))
  ...
  async takeOffer(
    ...
    const txInstruction = await this.program.methods.takeOffer()
    .accounts({...accounts})
    .instruction();
